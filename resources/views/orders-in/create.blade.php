...
@section('content')
    <div class="row">
    @if($clients->count() and $stores->count())
        <div class="col-md-12">
          ...
          <div class="box box-primary">
            ...
            <!-- /.box-header -->
            <!-- form start -->
            <form role="form" class="validate" action="{{route('orders-in.store')}}" method="POST" enctype="multipart/form-data">
            {{csrf_field()}}
    ...
    $(document).ready(function(){
        ...
        $openModal.click(function(){
            $.ajax({
                ...
                type:'POST',
                data:{
                    id: $storeId.find(':selected').val()
                },
                success:function(data){
                    options = ``;
                    $itemId.html('');
                    $(data.items).each(function(index , el){
                        options += `<option value="${el.item_id}"  data-id="${el.item_id}" data-price="${el.item.price}" data-max="${el.quantity}" >${el.item.name}</option>` 
                    })
